...
Open questions are recorded inline and collected in [§H](#h-open-questions-collected), sorted by **what has to happen to them**: **all five formerly-blocking questions are now decided** (OQ-11, OQ-10, OQ-6, OQ-7, OQ-3 — see §E.3, §D.5.3, §C.7c, §C.8 and §C.2), so implementation issues can be cut; five are decisions already taken with their dissent kept on record and are safely deferrable; two turned out not to be questions at all, but unmeasured constants with the benchmark harness already in the repo.
...
**Decision D-C2-1 — REVERSED by D-2026-09-13 (see OQ-3 below).** The stage does **not** complete: a `Suspended` child blocks completion whether or not it is required, and no `exit` cascade is fired at it. (The superseded decision was that the stage completes and the #179 cascade at `StageBehavior.cs:525-533` drives the Suspended child to Terminated. That cascade still applies to Available and Enabled children — only the Suspended case changed.) A **required** Suspended child blocked already, because Suspended ∉ {Disabled, Completed, Terminated, Failed}.
...
> **DECIDED — D-2026-09-13. OQ-3 is closed: a Suspended child blocks completion. D-C2-1 is reversed.**
>
> `EvaluateCompletion` gains a conjunct: **a stage does not complete while any child instance is `Suspended`**, required or not. No `exit` cascade is fired at a Suspended child.
>
> **Primary basis — Table 8.7's `Active` description (printed p.114):** "`Stage` instances in this state contain at least one `Stage` or `Task` instance in the Available, Enabled, Active, **Suspended** state, or autoComplete is set to FALSE." A stage holding a Suspended child is, by the spec's own state semantics, still Active. The `or autoComplete is FALSE` disjunct means this bites hardest in exactly the `autoComplete = TRUE` case at issue.
>
> The withdrawn justification for the old decision stays visible: an earlier draft argued "Table 8.12 permits the completion; Table 8.9 then *requires* the cascade". Table 8.9's `complete` rows give **Transition = `N/A`, To = `<impossible>`** — the *absence* of a prescribed cascade, not a mandate to fire one. Firing `exit` there was always the engine's own choice, never a spec requirement, which is what makes reversing it cheap.
>
> **Scope: `Suspended` only. #179's cascade for Available/Enabled children stands.** Table 8.7 names four states, and the maximal reading — treating all four as blocking — was considered and rejected for two reasons. First, it would make Table 8.12's `autoComplete = TRUE` column require all children terminal, which is its FALSE column's Branch 1; the spec would not state two columns that collapse into one. Second, and independently of Table 8.7, `Suspended` is the only one of the four carrying **preserved work**: Table 8.9's suspend/resume rows and Figure 8.3's history pseudo-state exist precisely so a suspended subtree can be restored to what it was. Terminating it destroys state the spec is explicit about preserving. An Available or Enabled child has nothing preserved to lose, so #179's cascade costs nothing and remains correct.
>
> So the rule is narrow, and rests on an argument that does not depend solely on reading a conjunct out of a descriptive sentence: **do not complete over preserved work.**
>
> **Consequences, both landing in P2:** `EvaluateCompletion`'s predicate changes, and so do the expected final states of any corpus scenario that suspends a non-required child. A stage blocked this way completes when the child resumes and reaches a terminal state, or when a worker terminates it explicitly — not on its own.
>
> What this settles either way: today's behaviour, where the outcome depends on the order two events arrived in, is unacceptable under **both** readings. R-C2 removes the ordering dependence; this decision fixes which answer it converges on.
...
> **DECIDED — D-2026-09-13. OQ-7 is closed: one sentry instance per criterion instance (D-C8-1 stands).**
>
> The rejected alternative — one shared sentry per owning-stage instance — is closer to today's topology and avoids duplicate IfPart evaluation. It was rejected because it requires satisfaction to fan out to referrers and consumption to be tracked per referrer, which reintroduces **"satisfied for whom?"**: a sentry satisfied for one referrer and not another, a state with no clean representation. That modelling gap is the shared root of C5 and #177, and the duplicate-evaluation cost is bounded, in-memory, and memoizable on `(expression body, contextRef, case-file version)` within a turn under I-2.
>
> **`forwardedSatisfaction` is part of this decision, not an implementation detail.** Per-criterion sentries give a repetition instance empty occurrence memory automatically — necessary, because today's shared latched sentry is exactly why a spawned instance can miss the satisfaction that created it. But empty memory *also* makes the new instance's criterion unsatisfiable from anything that already happened, so the triggering satisfaction must be forwarded explicitly (§B.3 operation 4, §C.6(a)). **Taking per-criterion without forwarding re-creates #177 rather than closing it** — the two ship together or not at all.
...
**Seam 3 — OQ-11 is decided: one engine, rewritten in place.** See below.

What remains genuinely atomic is built **in place on the feature branch** as a sequence of small PRs, each with its own tests. The new runtime is exercised by the in-process scenario runner from P2; the silo registration flips once, at P7, when the old grains are deleted. There is no second engine registered at any point.
...
| P3 | Sentry algebra (§C.8: **C4, M8, M9, M10, I7**), **M6a/b**, cross-stage + bottom-up (**M11**) | The sentry-focused corpus subset, run against the new engine |
...
> **DECIDED — D-2026-09-13. OQ-11 is closed: there is one engine, and this rewrite is it.**
>
> This question asked whether to build the new case grain as a dark branch or alongside the current one as a second grain type (`ICaseGrainV2`) chosen per case at `Create`. Both framings share a false premise — that a released implementation exists which has to be preserved, versioned against, or migrated from.
>
> **There is no v1.** Nothing has been released. Milestone 0.1.0 is literally named "Engine Complete" and is still open; §E.1 records no production traffic and no durable data beyond disposable local volumes. The current grain-per-plan-item topology is not a shipped version — it is unfinished work that turned out to be the wrong shape, and this rewrite is that work being corrected before 0.1.0 ships.
>
> So there is nothing to coexist with, nothing to migrate, and no compatibility surface to hold. `CaseGrain` is rewritten in place; the plan-item, sentry, case-file, planning-table and role grains are deleted as the phases that subsume them land. No `V2` type is ever registered and no per-case engine choice is ever recorded.
>
> **Git history is the archive.** If the current behaviour needs consulting, it is a `git show` away. Keeping a second live implementation in the silo to serve an occasional lookup is a permanent cost for an intermittent benefit.
>
> **What this does not change:** verification still begins at P2, not P7. Seam 1 stands — the in-process scenario runner is built first, and "how many of the 41 scenarios drive green" is a per-phase metric that must never go down. Seam 2 stands — the forced-atomic core is P2 + P3 + P7, with case file, timers and planning landing after cutover via outward delegation.
>
> **What it costs, stated plainly:** the corpus is not fully green on the feature branch between P2 and P7. That is the price of not running two engines, and it is why the rising green-scenario count is a required per-phase report rather than a nice-to-have — it is the only evidence the branch is converging. A phase that does not move that number needs explaining before the next one starts.
...
| **~~OQ-7~~ DECIDED** | C.8 | — | **Closed D-2026-09-13: per criterion instance.** Shared-per-stage reintroduces "satisfied for whom?", the modelling gap behind both C5 and #177. `forwardedSatisfaction` (§C.6(a)) ships with it — per-criterion without forwarding re-creates #177. |
| **~~OQ-3~~ DECIDED** | C.2 | — | **Closed D-2026-09-13: yes, it blocks. D-C2-1 reversed.** Table 8.7's `Active` description (printed p.114) names `Suspended`; Table 8.9's `complete` rows give `N/A`, so the old `exit` cascade was never spec-mandated. **Scoped to `Suspended` only** — #179's cascade for Available/Enabled stands, because the maximal reading would collapse Table 8.12's two columns and because only `Suspended` carries preserved work (Figure 8.3). |
