...
Two consequences that matter for the engine — and one place the reasoning has to be more careful than it first looks:

- `complete` is an **Active → Completed** transition only (Table 8.8). So the **no-entry-criteria** repetition trigger (§8's "without entry criteria" case, re-evaluated on `complete`/`terminate`) cannot originate from a genuinely Suspended instance — that determination only fires on the terminal transition itself, and a Suspended instance hasn't made one.
- The **entry-criteria** case is not confined the same way. `CaseFileItem` has no Suspended state at all (§8.3, Figure 8.1) and is never part of the `parent suspend`/`parent resume` propagation Table 8.9 defines for Stage/Task/Milestone/EventListener children — a CFI keeps updating regardless of whether the Stage that contains the item watching it is Suspended. So a CFI-sourced OnPart can still satisfy an entry criterion — and legitimately raise a repetition determination — for a sibling that is itself sitting Enabled or Suspended while its container is suspended. It is therefore **overstated** to say no repetition trigger can legitimately originate inside a genuinely Suspended Stage; only the complete/terminate-driven trigger is confined that way. The #198 completion gate still gets this right independent of which path raised the trigger, since it blocks on the repeating child's own live state rather than on an assumption about how that state was reached.
- Any repetition request the engine sees while its target sits Suspended is therefore either (a) a complete/terminate-driven trigger that really was earned before suspension and is merely late in delivery, or (b) a CFI-driven entry-criterion trigger that is legitimately concurrent with the suspension. Either way the spec's answer is the same: suspension preserves rather than discards, so the request must not be dropped. See [#178](https://github.com/en-gen/Wayfinder/issues/178), [#182](https://github.com/en-gen/Wayfinder/issues/182).
...
Table 8.9's `complete` rows mark the combination of a Completed Stage with a **Stage or Task** child in Available, Enabled, Active, or Suspended as **impossible**. Only Disabled, Failed, Completed, and Terminated Stage/Task children may coexist with a completed parent.

Milestone and EventListener children are a deliberate exception to that, not an oversight: Table 8.9's `complete` rows keep them in Available or Suspended rather than marking those combinations impossible — a Milestone/EventListener has no outstanding "work" for a completing container to wait on, so the spec lets it simply keep existing in whatever state it was in.

> **Implementation note.** This makes "non-terminal Stage/Task children survive a completing Stage" a conformance violation rather than a missing convenience — while surviving Milestone/EventListener children are conformant and expected. See [#179](https://github.com/en-gen/Wayfinder/issues/179).
...
| **RequiredRule** | on instantiation into Available; Table 8.8 also makes it a **MUST** re-evaluate again on `enable` and on `re-enable`, whenever the current value is FALSE | true → the parent Stage must not complete until this item is Completed, Terminated, Failed, or Disabled. Absent ⇒ false |
...
> **Known gap — not implemented.** The `enable`/`re-enable` re-evaluation above is a spec MUST (Table 8.8, printed p.115-116) that this engine does not perform: `EvaluateRequiredRule` is only invoked from the three Create paths (`StageBehavior`, `TaskBehavior`, `MilestoneBehavior`), and there is no `OnEntryFrom(Enable)`/`OnEntryFrom(Reenable)` registration anywhere that re-runs it. A RequiredRule that was FALSE at instantiation and becomes TRUE by the time the item is (re-)enabled is silently ignored — the parent Stage can complete over work the model now requires, and the completion cascade then exits it. Recorded here as a known conformance gap, not as implemented behavior; no issue number is filed for it yet.
...
>
> Residual window, stated rather than hand-waved: a repetition request published but never *delivered* leaves the child blocking completion. A handler-level failure inside a live cluster self-heals — the pulling agent retries and redelivers, which is why the #161 redelivery guard exists. Loss of the queued message itself does not, under the provider this engine is configured with today: `Wayfinder.Silo` uses in-memory streams over an in-memory PubSub store while grain journals are durable, so a silo restart can lose an undelivered repetition request that the child's own journal still records. **A durable stream provider alone would not close that gap.** Four things stand between here and closed: a durable stream provider (for the message itself); a durable PubSub store (the subscription registrations live there too, and it is registered in-memory — `AddMemoryGrainStorage("PubSubStore")` — independently of the stream provider); a re-subscription strategy for the sites that only re-arm `Resume`-only (`if (handles.Any())`) today, which would otherwise stay silently deaf forever after any restart even with the first two fixed; and an outbox, because the journal confirm and the stream publish are two separate writes today (`BaseBehavior.HandleTransitioned` confirms, then publishes) — a crash in that window durably records the transition with no publish ever attempted, which no amount of transport durability fixes on its own. Nothing in the completion check can supply any of that.
>
