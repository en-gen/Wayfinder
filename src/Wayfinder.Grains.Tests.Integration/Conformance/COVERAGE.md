...
| Table 8.9 complete rows — Milestone/EventListener column: a Milestone or EventListener child SURVIVES a completing parent (Available → Available, Suspended → Suspended), the deliberate asymmetry against the Stage/Task column above | `StageCompletionCascadeIntegrationTests.StageCompletionCascade__…MilestoneAndEventListenerChildrenRemainAvailable` (a non-required Milestone and a non-required UserEventListener, both left Available when their parent auto-completes, are still Available afterwards); unit-level twins at `MilestoneBehaviorTests`/`EventListenerBehaviorTests`' `HandleParentTransitioned__When_ParentCompleted__Then_NoEventAndNoTransition` (theory over Available and Suspended — no `ParentCompleted`, no transition); model-driven, not a `.cmmn` conformance scenario | Pinned (#216 phase 1 — characterization only, no production change; previously this asymmetry was stated in the row above's prose while only the Stage/Task half was pinned, so adding a parent-`complete` arm to `MilestoneBehavior`/`EventListenerBehavior` would have left the suite green) |
