...
A real **multi-container** Orleans cluster for Wayfinder (work item #49): Azurite plus 3 separate
`Flow.Silo` containers running the DEPLOYED clustering path (`Program.cs`
...
> **Removed 2026-09-13.** This stack used to carry a Postgres + Zitadel pair and an
> "Authenticated API walkthrough (First Light)" for `/api/v1/...` (work item #32/#33 sub-unit 4).
> Decision **D-2026-09-13** removed the HTTP ingress itself, so the OIDC infrastructure that
> existed only to authenticate it went too, along with `eval-authenticated.sh`. The ingress is
> rebuilt on [OhData](https://github.com/en-gen/OhData) after the case-grain granularity redesign
> (D-2026-08-01); the removed compose services and walkthrough are recoverable from git history if
> they are wanted as a starting point. See `docs/04-adversarial-review-2026-08.md` section 9.
...
- Out of scope for this stack: the HTTP ingress (removed - see the note at the top) and the
  Quartz/timer redesign (#31).
- Multi-tenant isolation is still proven automatically, at the grain and application layers, by
  `CaseTenantIsolationIntegrationTests` and `CaseCqrsIntegrationTests` in
  `src/Flow.Grains.Tests.Integration`. What went away with the ingress is
  `MultiTenantIsolationApiTests`, which proved those layers were correctly WIRED UP behind an
  authenticated HTTP surface - a property with nothing left to assert until an ingress returns,
  and one whose replacement belongs in the same PR that restores one.
